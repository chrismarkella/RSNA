...
class Preprocess:
    ...
    def label_set(self, directory):
        ...
        train_labels['Sub_type'] = train_labels['ID'].str.split("_",
                                                                n=3,
                                                                expand=True)[2]
        train_labels['PatientID'] = train_labels['ID'].str.split(
            "_", n=3, expand=True)[1]
    ...
    def window_image(self, pixel_array, window_center, window_width, intercept,
                     slope):
        """
        ...
        pixel_array_HU[
            pixel_array_HU < pixel_array_HU_min] = pixel_array_HU_min
        pixel_array_HU[
            pixel_array_HU > pixel_array_HU_max] = pixel_array_HU_max

        return pixel_array_HU

    def display_img_HU(self, pixel_img, window_center, window_width, intercept,
                       slope):
        """
        displays a image transformed into Hounsefield Units from pixel values
        """
        HU_img = window_image(pixel_img, window_center, window_width,
                              intercept, slope)
    ...
    def transform_all_pixel_arrays(self, dicom_file_lst):
        ...
        for dicom_file in dicom_file_lst:
            ...
            pixel_array_HU = self.window_image(pixel_array, window_center,
